...
export default function Shows({ name, color }) {
	...
	const sync = () => {
		...
		axios
			.post("/api/showtotaluniquecallers", {
				name: name,
			})
			.then(res => {
		...
		<>
			...
			<Grid item xs={10} md={3} lg={3}>
				...
				>
					...
					>
						<b>{name}</b> - total unique callers today
					</b>
					<img
						src={Des}
						style={{
